...
function initEventListener(fn) {
    window.addEventListener('message', event => {
        ...
        const fieldFocus = message?.fieldFocus;
        if (fieldFocus) {
            const elt = document.getElementById(fieldFocus);
            if (elt !== null) {
                elt.focus();
            }
        }
...
function fieldChanged(elt, val) {
    ...
    postCommandWithMap("validate", new Map([["___dirtyState", true]]));
}
...
function onShowPage() {
    ...
    postCommandWithMap("validate", new Map([["___dirtyState", false]]));
}
...
function postCommandWithMap(cmdid, customMap) {
    const map = customMap ? new Map([...wizardMap, ...customMap]) : wizardMap;
    vscode.postMessage({
        command: cmdid,
